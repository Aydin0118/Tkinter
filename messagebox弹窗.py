...
def minhh():#设置关闭窗口的回调函数设置成点击关闭窗口没用
    b=tk.messagebox.askokcancel(title='提示',message='你确定要退出吗？')
    if b:
        win.destroy()
    else:
        tk.messagebox.showinfo(title='提示',message='谢谢')#提示信息

    
win.protocol('WM_DELETE_WINDOW',minhh) #设置关闭窗口的回调函数
def hit_me():
    #tk.messagebox.showinfo(title='message',message='hello world')#值为OK
    print(tk.messagebox.showinfo(title='message',message='hello world'))#既可以弹窗也可以在终端显示取值
    
    #tk.messagebox.showwarning(title='message',message='warning')值为OK
    print(tk.messagebox.showwarning(title='message',message='warning'))

    #tk.messagebox.showerror(title='message',message='error')值为OK
    print(tk.messagebox.showerror(title='message',message='error'))

    #tk.messagebox.askquestion(title='message',message='question')值为YES或NO
    print(tk.messagebox.askquestion(title='message',message='question'))

    #tk.messagebox.askokcancel(title='message',message='okcancel')值为True或False
    print(tk.messagebox.askokcancel(title='message',message='okcancel'))
    #tk.messagebox.askyesno(title='message',message='yesno') 值为True或False

    print(tk.messagebox.askyesno(title='message',message='yesno')) #HD
    i=(tk.messagebox.askyesno(title='message',message='yesno')) #一共俩次askyesno,print可以输出窗口，将窗口赋值也可以
